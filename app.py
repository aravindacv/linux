# =========================
# Bash Tutorial — Complete Linux Coach (Full-Width Layout)
# =========================
import subprocess
import os
import tempfile
...
<style>
    ...
    .card { border:1px solid #e5e7eb; border-radius:12px; padding:18px 20px; background:#fff; margin:10px 0; line-height: 1.6; }
    .card .title { font-weight:700; margin-bottom:8px; font-size: 1.1rem; color: #0f172a;}
    ...
    .term-box {
        ...
        padding: 20px; font-family: 'Consolas', 'Courier New', monospace; font-size: 14px;
...
# -------------------------------------------------------
# COMPLETE DATABASE (Detailed Explanations Added)
# -------------------------------------------------------
@dataclass
class Lesson:
    id: str
...
# -------------------------------------------------------
# Main Application Layout (FULL WIDTH - TOP TO BOTTOM)
# -------------------------------------------------------
def main():
    init_bash_sandbox()
    ...
    # ==========================================
    # TOP SECTION: SYLLABUS, THEORY, MCQ
    # ==========================================
    st.markdown("### 📚 Bash Tutorial Syllabus")
    ...
    if selected_label.startswith("    ⚡"):
        ...
        st.info("👈 Select a specific lesson from the dropdown above to view its detailed explanation, run examples, and take the quiz.")

    # ==========================================
    # BOTTOM SECTION: PRACTICE TERMINAL
    # ==========================================
    st.markdown("---")
    st.markdown("### 🖥️ Practice")
    st.caption(f"Type your own commands below to practice. Working Directory: `{st.session_state.sandbox_dir}`")
    
    # Render Terminal History
    term_html = "<div class='term-box'>"
    if not st.session_state.term_history:
        term_html += "<span style='color:#666'>Terminal output will appear here...</span>"
    else:
