...
def render_mcq(questions: List[Dict[str, Any]], key_prefix: str):
    """Renders MCQs with instant feedback (From CoderBuddy Reference)"""
...
# -------------------------------------------------------
# Complete Lesson Database (Includes MCQs)
# -------------------------------------------------------
@dataclass
class Lesson:
    id: str
    ...
    theory: str
    demo_cmd: str
    quiz: List[Dict[str, Any]] # Added Quiz field
...
def get_all_lessons() -> List[Lesson]:
    return [
        ...
        Lesson("variables", "SCRIPTING", "Variables", "Store data. **No spaces around `=`**. Access with `$`.", "MY_OS=\"Linux\" && echo \"I love $MY_OS\"", 
               [{"q": "How do you read a variable in Bash?", "options": ["NAME", "$NAME", "%NAME%", "&NAME"], "answer": 1, "explain": "The dollar sign $ must precede the variable name to read its value."}]),
...
def main():
    ...
    # ==========================================
    # LEFT COLUMN: LESSONS, MCQs, & CONTENT
    # ==========================================
    with left_col:
        ...
        if selected_label.startswith("    ⚡"):
            ...
            # MCQ Section (If quiz exists for this lesson)
    ...
    with right_col:
        ...
        # THE FIXED INPUT (No infinite loop)
